...
run_cmake_build() {
    ...
    cmake src/rl_ITRI/ -B cmake_build -DUSE_CMAKE=ON
...
run_mujoco_build() {
    ...
    cmake src/rl_ITRI/ -B cmake_build -DUSE_CMAKE=ON -DUSE_MUJOCO=ON
    cmake --build cmake_build -j$(nproc 2>/dev/null || echo 4)
    ...
    if [ -d "$package_dir" ]; then
        if [ -f "$package_dir/package.ros1.xml" ] && [ -f "$package_dir/package.ros2.xml" ]; then
            ...
            if [[ "$ROS_DISTRO" == "noetic" ]]; then
                ...
                return 0
            elif [[ "$ROS_DISTRO" == "foxy" || "$ROS_DISTRO" == "humble" || "$ROS_DISTRO" == "galactic" ]]; then
