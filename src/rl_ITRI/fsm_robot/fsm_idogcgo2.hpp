...
#ifndef IDOGCGO2_FSM_HPP
#define IDOGCGO2_FSM_HPP

#include "fsm.hpp"
#include "rl_sdk.hpp"

namespace idogcgo2_fsm
{
...
public:
    ...
    void Enter() override
    {
        ...
        catch (const std::exception& e)
        {
            ...
            std::cout << "Current config: " << rl.params.config_node << std::endl;
        }
    }
    ...
    void Exit() override
    {
        rl.rl_init_done = false;
        // 直接覆蓋成新的 Node
        // rl.params.config_node = YAML::Node(YAML::NodeType::Map);
...
class RLFSMStateRLRobotLabLocomotion : public RLFSMState
{
public:
    RLFSMStateRLRobotLabLocomotion(RL *rl) : RLFSMState(*rl, "RLFSMStateRLRobotLabLocomotion") {}
...
class RLFSMStateRLExtremeParkourLocomotion : public RLFSMState
{
public:
    RLFSMStateRLExtremeParkourLocomotion(RL *rl) : RLFSMState(*rl, "RLFSMStateRLExtremeParkourLocomotion") {}
    ...
    void Enter() override
    {
        ...
        // read params from yaml
        rl.config_name = "extreme_parkour";
        std::string robot_config_path = rl.robot_name + "/" + rl.config_name;
        try
        {
            rl.InitRL(robot_config_path);
...
class IdogcGo2FSMFactory : public FSMFactory
{
public:
    IdogcGo2FSMFactory(const std::string& initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<idogcgo2_fsm::RLFSMStatePassive>(rl);
        else if (state_name == "RLFSMStateGetUp")
            return std::make_shared<idogcgo2_fsm::RLFSMStateGetUp>(rl);
        else if (state_name == "RLFSMStateGetDown")
            return std::make_shared<idogcgo2_fsm::RLFSMStateGetDown>(rl);
        else if (state_name == "RLFSMStateRLLocomotion")
            return std::make_shared<idogcgo2_fsm::RLFSMStateRLLocomotion>(rl);
        else if (state_name == "RLFSMStateRLRobotLabLocomotion")
            return std::make_shared<idogcgo2_fsm::RLFSMStateRLRobotLabLocomotion>(rl);
        else if (state_name == "RLFSMStateRLExtremeParkourLocomotion")
            return std::make_shared<idogcgo2_fsm::RLFSMStateRLExtremeParkourLocomotion>(rl);
        return nullptr;
    }
    std::string GetType() const override { return "idogcgo2"; }
    std::vector<std::string> GetSupportedStates() const override
    {
        return {
            ...
            "RLFSMStateRLRobotLabLocomotion",
            "RLFSMStateRLExtremeParkourLocomotion"
        };
    }
...
REGISTER_FSM_FACTORY(IdogcGo2FSMFactory, "RLFSMStatePassive")

#endif // IDOGCGO2_FSM_HPP
