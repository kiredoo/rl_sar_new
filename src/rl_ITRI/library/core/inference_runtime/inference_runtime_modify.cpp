...
std::vector<float> TorchModel::forward(const std::vector<std::vector<float>>& inputs)
{
    ...
    try
    {
        ...
        //下面为新代码
        //获取子模块
        torch::jit::script::Module actor = model_.attr("actor").toModule();
        torch::jit::script::Module encoder = model_.attr("estimator").toModule();
        
        // 1. 通过encoder得到predict_tensor
        torch::Tensor predict_tensor = encoder.forward({input_tensor}).toTensor();
        
        // 2. 分割为vel_tensor和latent_tensor
        torch::Tensor vel_tensor = predict_tensor.slice(1, 0, 3);
        torch::Tensor latent_tensor = predict_tensor.slice(1, 3);
        
        // 3. 对latent_tensor进行L2归一化
        torch::Tensor norm = torch::norm(latent_tensor, 2, 1, true);
        latent_tensor = latent_tensor / norm;
        
        // 4. 准备obs_tensor输入
        int num_obs_current = 45; // 当前观测维度
        torch::Tensor obs_current = input_tensor.slice(1, 0, num_obs_current);
        torch::Tensor obs_tensor = torch::cat({obs_current, vel_tensor, latent_tensor}, 1);
        
        // 5. 通过actor得到action
        torch::Tensor action_tensor = actor.forward({obs_tensor}).toTensor();
        
        // 6. 转换为std::vector<float>
        action_tensor = action_tensor.detach().cpu().flatten();
        std::vector<float> action;
        
        float* data_ptr = action_tensor.data_ptr<float>();
        long num_elements = action_tensor.numel();
        action.insert(action.end(), data_ptr, data_ptr + num_elements);
