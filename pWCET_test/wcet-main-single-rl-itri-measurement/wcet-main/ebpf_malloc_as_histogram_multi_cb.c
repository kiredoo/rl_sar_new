...
#define MAX_CB_NAME_LEN 128  // maximum length of demangled function names

struct histogram_t {
  char cb_name[MAX_CB_NAME_LEN];
  s32 bit_len_count[MAX_BIT_LEN + 1];
  u64 total_allocated_bytes;
};

BPF_RINGBUF_OUTPUT(g_histogram_rbo, 8);          // 8 page
BPF_HASH(g_histogram, u64, struct histogram_t);  // pid_tgid -> histogram_t
...
int malloc_in(struct pt_regs *ctx) {
  ...
  if (is_active && (*is_active)) {
    struct histogram_t *histogram = g_histogram.lookup(&pid_tgid);
    if (histogram) {
...
int cb_in_{{loc.mangled_name}}(struct pt_regs *ctx) {
  ...
  if (histogram) {
    ...
    const char *cb_name = "{{demangled_name}}";
    for (s32 i = 0; i < MAX_CB_NAME_LEN && cb_name[i] != '\0'; i++) {
      histogram.cb_name[i] = cb_name[i];
    }
    g_histogram.update(&pid_tgid, &histogram);
  }
  return 0;
}
